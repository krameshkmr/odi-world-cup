...
rmse_3
```
This table shows all of the different *RMSE* values after removing one predictor variable from the training set without the `mo_sr` and `bowlers_used` predictor variables. Based on this table, the lowest *RMSE* is around 0.136 which is the 5th row and corresponds to the `to_bf_pct` predictor variable. Now, we will permanently remove this variable from the training set and repeat this process again to keep minimizing the *RMSE*.
...
This table shows all of the different *RMSE* values after removing one predictor variable from the training set without the `mo_sr`, `bowlers_used`, and `to_bf_pct` predictor variables. Based on this table, the lowest *RMSE* is around 0.1339 which is the 7th row and corresponds to the `pct_6s` predictor variable. Now, we will permanently remove this variable from the training set and repeat this process again to keep minimizing the *RMSE*.
...
rmse_5
```
This table shows all of the different *RMSE* values after removing one predictor variable from the training set without the `mo_sr`, `bowlers_used`, `to_bf_pct`, and `pct_6s` predictor variables. Based on this table, the lowest *RMSE* is around 0.129 which is the 4th row and corresponds to the `mo_mins_pct` predictor variable. Now, we will permanently remove this variable from the training set and repeat this process again to keep minimizing the *RMSE*.
...
rmse_6
```
This table shows all of the different *RMSE* values after removing one predictor variable from the training set without the `mo_sr`, `bowlers_used`, `to_bf_pct`, `pct_6s`, and `mo_mins_pct` predictor variables. Based on this table, the lowest *RMSE* is 0.128862 which is the 4th row but this value is actually higher than the previous lowest *RMSE* value. This means we can stop the process and create our final model with the collection of predictor variables our backwards step-wise selection process determined. 
...
Our final model, after taking out `mo_sr`, `bowlers_used`, `to_bf_pct`, `pct_6s`, and `mo_mins_pct` predictor variables, has an *Adjusted R-Squared* of 0.857 and an *RMSE* value of 0.129. This means our model is able to account for 85.7% of the variation in `mov` and the predicted values are generally 12.9% off od the observed values for `mov`. 
...
rmse/(max(odi$mov) - min(odi$mov))
```
To analyze our final model, we train the model with the training set with the collection of predictor variables determined by our backwards step-wise selection process and the test the model on the entire dataset. The model's *Adjusted R-Squared* is 0.857 which means we are able to account for 85.7% of the variation in `mov` based on our predictor variables. The *RMSE* is around 0.138 which means our predicted values generally differ from the observed values of `mov` by 13.8%. The normalized *RMSE* is around 0.08. The normalized *RMSE* gives an *RMSE* value that depicts the error based on the range of our dataset. Using this value, this means our predicted values differ from the observed values of `mov` by around 8%. Before doing the backwards step-wise selection process, the first model we made had an *Adjusted R-Squared* of 0.881 and an *RMSE* of around 0.153. Comparing our final model to our first one, our *Adjusted R-Squared* decreased by around 3% and the *RMSE* decreased by around 1%. The decrease of the *Adjusted R-Squared* value by 3% is not that significant because we were able to improve our *RMSE* by a 1% which is more important. 

```{r}
model_coefs <- coef(model_odi)
final_set %>% 
  ggplot(aes(x = predictions, y = mov)) + 
  geom_point() +
